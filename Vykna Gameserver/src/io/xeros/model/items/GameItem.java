...
public class GameItem implements ItemInterface {
	...
	/** Sets attributes and recomputes hash (call this any time attrs changes) */
	public GameItem setAttrs(ItemAttributes attrs) {
		this.attrs = attrs;
		this.attrsHash = (attrs == null) ? 0 : attrs.computeHash();
		return this;
	}

	/** True if these two items can stack into one slot */
	public boolean stacksWith(GameItem other) {
		if (other == null) return false;
		if (this.id != other.id) return false;

		// both no attrs
		if (!this.hasAttrs() && !other.hasAttrs()) return true;

		// both have attrs, must match hash + fields
		if (this.attrsHash != other.attrsHash) return false;
		return this.attrs != null && other.attrs != null && this.attrs.equalsAttrs(other.attrs);
	}
	...
	public GameItem copy() {
		GameItem c = new GameItem(id, amount);
		if (this.attrs != null) {
			c.setAttrs(this.attrs.copy());
		}
		return c;
	}
