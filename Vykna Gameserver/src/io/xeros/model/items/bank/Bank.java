...
public class Bank {
	...
	public boolean withdrawFromSlot(int interfaceId, int itemId, int slot, int amount) {
		...
		// Search container: do NOT trust slot mapping (filtered list).
		if (interfaceId == BANK_SEARCH_CONTAINER) {
			BankTab tab = getBankForInput(interfaceId, itemId);
			if (tab == null) return false;

			// Find the first matching itemId in that tab and withdraw that exact stack
			for (int i = 0; i < tab.size(); i++) {
				BankItem bi = tab.getItem(i);
				if (bi != null && bi.getId() == itemId + 1) {
					return withdrawExact(tab, i, amount);
				}
			}
			return false;
		}

		BankTab tab = getBankTabForInterfaceId(interfaceId);
		if (tab == null) return false;

		return withdrawExact(tab, slot, amount);
	}
	public int getAllButOneFromSlot(int interfaceId, int slot) {
		BankTab tab = getBankTabForInterfaceId(interfaceId);
		if (tab == null) return -1;

		BankItem item = tab.getItem(slot);
	...
	public boolean withdraw(int interfaceId, int itemId, int amount) {
		...
		// Search container is filtered; itemId is the only reliable key here.
		if (interfaceId == BANK_SEARCH_CONTAINER) {
			for (int i = 0; i < tab.size(); i++) {
				BankItem bi = tab.getItem(i);
				if (bi != null && bi.getAmount() > 0 && bi.getId() == itemId + 1) {
					return withdrawExact(tab, i, amount);
				}
			}
			return false;
		...
		// Normal tab: still itemId-based (legacy). Pick the first matching real stack.
		for (int i = 0; i < tab.size(); i++) {
			BankItem bi = tab.getItem(i);
			if (bi != null && bi.getAmount() > 0 && bi.getId() == itemId + 1) {
				return withdrawExact(tab, i, amount);
	...
	private boolean withdrawExact(BankTab tab, int fromSlot, int amount) {
		if (tab == null) return false;
		if (fromSlot < 0 || fromSlot >= tab.size()) return false;
		if (amount == -1) return false;
		...
		// ✅ The key line: remove the EXACT bank slot the user clicked
		player.getItems().removeFromBankSlot(tab, fromSlot, amount, true);
	...
	public boolean hasRoomFor(BankItem bankingItem) {
		...
		for (BankTab bankTab : bankTabs) {
			for (GameItem bankedItem : bankTab.getItems()) {
				if (bankedItem.stacksWith(bankingItem)) {
