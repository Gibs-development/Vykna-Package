...
public class PlayerAssistant {
	...
	public void sendSound(int soundId) {
		...
		c.getOutStream().writeWord(soundId); // UShort
		c.getOutStream().writeByte(1);       // SOUND
		c.getOutStream().writeWord(0);       // entitySource = 0 (local)
		c.flushOutStream();
	}
	...
	public void sendAreaSound(int soundId, io.xeros.model.SoundType soundType, int distance) {
		if (soundId <= 0 || c.getOutStream() == null) {
			return;
		}
		c.getOutStream().createFrame(239);
		c.getOutStream().writeWord(soundId);
		c.getOutStream().writeByte(soundType.ordinal());
		c.getOutStream().writeWord(distance);
		c.flushOutStream();
	}
...
//	updateConfig(c, spell);
	...
	public void sendAttrDbgBegin(String label) {
		sendClientCommand(":attrdbg: begin " + (label == null ? "" : label));
	}

	public void sendAttrDbgEnd() {
		sendClientCommand(":attrdbg: end");
	}

	public void sendAttrDbgSlot(int interfaceId, int slot, int itemId, int hash,
								int rarity, int perk1, int perk1Rank, int perk2, int perk2Rank) {
		sendClientCommand(":attrdbg: if=" + interfaceId
				+ " slot=" + slot
				+ " id=" + itemId
				+ " hash=" + hash
				+ " r=" + rarity
	...
	/** Dumps the player's inventory attrs using piggyback opcode 126 client-command parsing. */
// PlayerAssistant.java
	public void sendInventoryAttrDbg(Player c, int interfaceId) {
		sendAttrDbgBegin("inv");
		for (int slot = 0; slot < 28; slot++) {
			...
			int p1 = (a == null ? 0 : a.perk1);
			int p1r = (a == null ? 0 : a.perk1Rank);
			int p2 = (a == null ? 0 : a.perk2);
			int p2r = (a == null ? 0 : a.perk2Rank);

			sendAttrDbgSlot(interfaceId, slot, id, h, rarity, p1, p1r, p2, p2r);
		}
		sendAttrDbgEnd();
	}


	/** Dumps the player's equipment attrs using piggyback opcode 126 client-command parsing. */
	public void sendEquipmentAttrDbg(Player c) {
		sendAttrDbgBegin("equip");
		for (int slot = 0; slot < c.playerEquipment.length; slot++) {
			int id = c.playerEquipment[slot];
			if (id < 0) id = 0;

			ItemAttributes a = c.playerEquipmentAttrs[slot];
			int h = c.playerEquipmentAttrHash[slot];
			...
			sendAttrDbgSlot(1688, slot, id, h, rarity, p1, p1r, p2, p2r);
		}
		sendAttrDbgEnd();
	}
	...
	/**
	 * Dumps bank tab attrs for a specific bank container interface.
	 * Uses piggyback opcode 253 (client command parsing) with an explicit interface id (if=...).
	 */
	public void sendBankTabAttrDbg(Player c, int interfaceId, io.xeros.model.items.bank.BankTab tab) {
		if (tab == null) {
			return;
		}
		sendAttrDbgBegin("bank");
		for (int slot = 0; slot < tab.size(); slot++) {
			io.xeros.model.items.bank.BankItem bi = tab.getItem(slot);
			if (bi == null) {
				sendAttrDbgSlot(interfaceId, slot, 0, 0, 0, 0, 0, 0, 0);
				continue;
			}
			int id = bi.getId() - 1; // client uses base item id
			int hash = bi.getAttrsHash();
			io.xeros.model.items.ItemAttributes a = bi.getAttrs();
			int rarity = (a == null ? 0 : a.rarityId);
			int p1 = (a == null ? 0 : a.perk1);
			int p1r = (a == null ? 0 : a.perk1Rank);
			int p2 = (a == null ? 0 : a.perk2);
			int p2r = (a == null ? 0 : a.perk2Rank);
			sendAttrDbgSlot(interfaceId, slot, id, hash, rarity, p1, p1r, p2, p2r);
		}
		sendAttrDbgEnd();
	}

	public void sendClientCommand(String s) {
...
//	public void sendInvAndEquipAttrs() {
//		sendContainerAttrs(3214, c.playerItemAttrs, c.playerItemAttrHash, 28);
//		sendContainerAttrs(1688, c.playerEquipmentAttrs, c.playerEquipmentAttrHash, c.playerEquipment.length);
//	}

//	public void sendContainerAttrs(int interfaceId, ItemAttributes[] attrs, int[] hashes, int size) {
//		if (c.getOutStream() == null) return;
//		if (attrs == null || hashes == null) return;
//
//		int max = Math.min(attrs.length, hashes.length);
//		int sendSize = Math.min(size, max);
//		if (sendSize < 0) sendSize = 0;
//
//		System.out.println("[ATTR] sending iface=" + interfaceId + " size=" + sendSize + " player=" + c.getLoginName());
//
//		c.getOutStream().createFrameVarSizeWord(250);
//		c.getOutStream().writeUnsignedWord(interfaceId);
//		c.getOutStream().writeUnsignedWord(sendSize);
//
//		for (int slot = 0; slot < sendSize; slot++) {
//			ItemAttributes a = attrs[slot];
//			int h = hashes[slot];
//
//			int flags = (a != null && h != 0) ? 1 : 0;
//			c.getOutStream().writeByte(flags);
//
//			if (flags == 1) {
//				c.getOutStream().writeDWord(h);
//				c.getOutStream().writeByte(a.rarityId & 0xFF);
//				c.getOutStream().writeUnsignedWord(a.perk1 & 0xFFFF);
//				c.getOutStream().writeByte(a.perk1Rank & 0xFF);
//				c.getOutStream().writeUnsignedWord(a.perk2 & 0xFFFF);
//				c.getOutStream().writeByte(a.perk2Rank & 0xFF);
//			}
//		}
//
//		c.getOutStream().endFrameVarSizeWord();
...
//				43, 43, 0, 0);
	...
	private void startTeleportFinal(int x, int y, int height, String teleportType, boolean homeTeleport) {
		...
		c.getPA().sendSound(Sfx.TELEPORT_GENERIC);
		resetVariables();
	...
	/**
	 * Send a MIDI music track to the client (packet 74).
	 * Client reads this as little-endian (method434), so we must writeWordBigEndian.
	 *
	 *
	 */
	...
	public void sendMusic(int songId) {
		...
		c.getOutStream().writeWordBigEndian(songId); // matches your client side
	}

	public void updateRegionalMusic() {
		int track = RegionalMusic.resolve(c);
		if (track == c.lastMusicTrack) return;
		c.lastMusicTrack = track;
		sendMusic(track);
	}
