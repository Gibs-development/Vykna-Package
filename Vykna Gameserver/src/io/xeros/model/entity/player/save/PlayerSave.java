     ...
     */
                    ...
                    if (spot > -1) {
                        ...
                        switch (ReadMode) {
                            ...
                            case 2:
                                ...
                                } else if (token.equals("Ringofrunerogue")) {
                                    p.setRingOfKoranesColletion(RingOfKoranes.CollectionLocation.valueOf(token2));

                                } else if (token.equals("RingofrunerogueImbue")) {
                            ...
                            case 6:
                                if (token.equals("character-item")) {
                                    int slot = Integer.parseInt(token3[0]);
                                    p.playerItems[slot] = Integer.parseInt(token3[1]);
                                    p.playerItemsN[slot] = Integer.parseInt(token3[2]);
                                    ...
                                    if (rarityId != 0 || perk1 != 0 || perk2 != 0) {
                                        ...
                                        p.playerItemAttrs[slot] = null;
                                        p.playerItemAttrHash[slot] = 0;
                                    }
                                }
                            ...
                            case 7:
                                if (token.equals("bank-tab")) {
                                    ...
                                    // Vykna attrs:
                                    // Old format: tabId itemId amount [rarity]
                                    // New format: tabId itemId amount rarity perk1 perk1Rank perk2 perk2Rank
                                    BankItem bankItem = new BankItem(itemId, itemAmount);
                                    if (token3.length >= 4) {
                                        byte rarityId = Byte.parseByte(token3[3]);
                                        if (rarityId != 0) {
                                            ItemAttributes attrs = new ItemAttributes();
                                            attrs.rarityId = rarityId;
                                            if (token3.length >= 8) {
                                                attrs.perk1 = Short.parseShort(token3[4]);
                                                attrs.perk1Rank = Byte.parseByte(token3[5]);
                                                attrs.perk2 = Short.parseShort(token3[6]);
                                                attrs.perk2Rank = Byte.parseByte(token3[7]);
                                            }
                                            bankItem.setAttrs(attrs);
                                        }
                                    }

                                    // IMPORTANT: don't use BankTab.add here, it may stack by ID only.
                                    // We want to preserve distinct stacks that differ by attrs.
                                    p.getBank().getBankTab()[tabId].getItems().add(bankItem);
                                }
        ...
        public static boolean saveGameInstant(Player p) {
            ...
            try {
                ...
                characterfile.newLine();
                characterfile.write("Ringofrunerogue = " + p.getRingOfKoranesCollection());
                characterfile.newLine();
                characterfile.write("RingofrunerogueImbue = " + p.getRingOfKoranesImbueCollection());
                characterfile.newLine();
                    ...
                    for (int index = 0; index < bankTab.getItems().size(); index++) {
                        ...
                        if (item != null) {
                            byte rarityId = 0;
											...
											short perk2 = 0;
											byte perk2Rank = 0;
                            if (item.hasAttrs() && item.getAttrs() != null) {
												ItemAttributes a = item.getAttrs();
												rarityId = a.rarityId;
												perk1 = a.perk1;
												perk1Rank = a.perk1Rank;
												perk2 = a.perk2;
											...
											characterfile.write("bank-tab = " + bankTabIndex
													+ "\t" + item.getId()
													+ "\t" + item.getAmount()
													+ "\t" + rarityId
													+ "\t" + perk1
