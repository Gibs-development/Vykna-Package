...
public class Player extends Entity {
   ...
   private boolean printAttackStats = Configuration.DEBUG_MODE;
    ...
    private VyknaProgressionPlayerState vyknaProgressionState = new VyknaProgressionPlayerState();
    ...
    // --- Music ---
    public int lastMusicTrack = -2;
    public boolean regionalMusicAuto = true;
    ...
    // ---- Vykna item attributes for inventory slots ----
    public ItemAttributes[] playerItemAttrs = new ItemAttributes[28];
    public int[] playerItemAttrHash = new int[28];
    ...
    /**
     * Combat-state: refreshed on attacks/hits and expires after IN_COMBAT_TIMER.
     * This is used for logout lock + combat idle pose.
     */
    private long combatStateUntilMs = 0L;
    private boolean combatStandActive = false;

    public void refreshCombatState() {
        combatStateUntilMs = System.currentTimeMillis() + Configuration.IN_COMBAT_TIMER;
    }

    public boolean isInCombatState() {
        return System.currentTimeMillis() < combatStateUntilMs;
    }

    public int combatStateSecondsRemaining() {
        long rem = combatStateUntilMs - System.currentTimeMillis();
        if (rem <= 0) return 0;
        return (int) ((rem + 999) / 1000);
    }

    /**
     * Applies / removes combat stand animation while combat-state is active.
     * Only runs if Configuration.COMBAT_ACTIVE_STAND_ANIMATION > 0.
     */
    private void processCombatStand() {
        int combatStand = Configuration.COMBAT_ACTIVE_STAND_ANIMATION;
        if (combatStand <= 0) {
            // feature disabled
            combatStandActive = false;
            return;
        }

        boolean inCombat = isInCombatState();

        if (inCombat) {
            if (!combatStandActive || playerStandIndex != combatStand) {
                combatStandActive = true;
                playerStandIndex = combatStand;
        ...
        // Leaving combat: restore correct stand for current weapon/unarmed
        if (combatStandActive) {
            combatStandActive = false;
            ...
            if (playerEquipment[playerWeapon] == -1) {
                ...
                playerTurnIndex = 823;
                playerWalkIndex = 819;
                playerTurn180Index = 820;
                playerTurn90CWIndex = 821;
                playerTurn90CCWIndex = 822;
                playerRunIndex = 824;
            } else {
                io.xeros.content.combat.melee.MeleeData.setWeaponAnimations(this);
            }
    ...
    private int wrath;                 // 0..100
    private boolean wrathUpdate;        // true when changed, until sent
    ...
    public boolean isWrathUpdate() { return wrathUpdate; }
    public void clearWrathUpdate() { wrathUpdate = false; }
...
//        }
    ...
    public VyknaProgressionPlayerState getVyknaProgressionState() {
        return vyknaProgressionState;
    }

    public void setVyknaProgressionState(VyknaProgressionPlayerState vyknaProgressionState) {
        this.vyknaProgressionState = vyknaProgressionState;
    }
