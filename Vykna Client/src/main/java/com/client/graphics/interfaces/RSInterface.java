...
public class RSInterface {
	...
	private static final double NORMAL_ZOOM_MULTIPLIER = 1.25;
	...
	public static void unpack(StreamLoader streamLoader, TextDrawingArea textDrawingAreas[],
		...
		interfaceCache = new RSInterface[Math.max(j + 80000, 230000)];
	...
	private static int computeZoomForSpan(int span, int targetW, int targetH) {
		...
		double multiplier = span >= HUGE_SPAN_THRESHOLD ? HUGE_ZOOM_MULTIPLIER : NORMAL_ZOOM_MULTIPLIER;
		zoom = (int) Math.round(zoom * multiplier);
